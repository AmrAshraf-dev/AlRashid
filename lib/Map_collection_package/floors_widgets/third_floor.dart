...
import 'package:alrashid_mall_app/Map_collection_package/view/widgets/thirdfloor_ui.dart';
import 'package:alrashid_mall_app/Views/UI/Indoor_maps/from_to_third.dart';
...
class ThirdFloor extends StatefulWidget {
  ...
  final int? toId;
  final int? floor;
  const ThirdFloor({Key? key, this.fromId, this.toId, this.floor})
      : super(key: key);
...
class _ThirdFloorState extends State<ThirdFloor> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance?.addPostFrameCallback((_) async {
      await _blocPositions.fetchAllPositionsData(context, 4);
      // .whenComplete(() {
      //   setState(() {});
      // });
      ...
      // await _blocRoutes
      //     .fetchAllRoutesData(context, widget.fromId ?? 0, widget.toId ?? 0, 4)
      //     .whenComplete(() {
      //   setState(() {});
      // });
    ...
    return Scaffold(
      ...
      body: ClipRRect(
        ...
        child: GestureDetector(
          onTap: () {
            Navigator.push(
                context, MaterialPageRoute(builder: (c) => FromToThird()));
          },
          child: Container(
