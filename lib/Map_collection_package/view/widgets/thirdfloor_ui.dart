...
class ThirdFloorUI extends StatefulWidget {
  const ThirdFloorUI({Key? key}) : super(key: key);

  @override
  State<ThirdFloorUI> createState() => _ThirdFloorUIState();
}

List<ShopPositionModel> shopPointersLocationsThirdFloor = [];
List<RoutePositionModel> routePositionsThirdFloor = [];
LinesModel? lineThirdFloor;
List<LinesModel> linesThirdFloor = [];
pointersOnImage(Size? size) {
  List<Widget> returnList = [
    Image.asset(
      'assets/images/third_floor-copy.jpg',
      fit: BoxFit.contain,
    ),
  ];

  for (var element in shopPointersLocationsThirdFloor) {
    ...
    print('shop POINTERS LOCATIONS :: ==>> :$shopPointersLocationsThirdFloor');
  }

  return returnList;
}

class _ThirdFloorUIState extends State<ThirdFloorUI>
  ...
  void initState() {
    ...
    controller.forward();
    setState(() {
      routePositionsThirdFloor = PositionsController.routePositionModel;
      shopPointersLocationsThirdFloor = PositionsController.shopPostionModel;
      linesThirdFloor = PositionsController.lines;
      lineThirdFloor = PositionsController.line;
      print("shop pointers locs");
      print(shopPointersLocationsThirdFloor);
      print("lines");
      print(lineThirdFloor);
    });
  }
  ...
  Widget build(BuildContext context) {
    MyPainter painter = MyPainter(routePositionsThirdFloor);
    _onRouteSelect(TapUpDetails details) {
      var x = details.localPosition.dx.toInt();
      var y = details.localPosition.dy.toInt();
      Size size = currentSize;
      dynamic lineId = lineThirdFloor?.id;

      setState(() {
        routePositionsThirdFloor.add(RoutePositionModel(
            x: x,
            y: y,
            screenWidth: size.width.toInt(),
            screenHeight: size.height.toInt(),
            lineId: lineId));
        print('lines modell iddd : ${lineThirdFloor?.id ?? 0}');
      });
    ...
    onTapDown(TapUpDetails details) {
      ...
      setState(() {
        shopPointersLocationsThirdFloor.add(ShopPositionModel(
                            ...
                            children: [
                              Expanded(
                                child: GestureDetector(
                                  onTap: () {
                                    setState(() async {
                                      shopPointersLocationsThirdFloor
    ...
    return CustomPaint(
        ...
        child: Container(
          child: GestureDetector(
            // onTapUp: (TapUpDetails details) => onTapDown(details),
