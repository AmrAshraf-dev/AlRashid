...
class SecondFloorUI extends StatefulWidget {
  const SecondFloorUI({Key? key}) : super(key: key);

  @override
  State<SecondFloorUI> createState() => _SecondFloorUIState();
}

List<ShopPositionModel> shopPointersLocationsSecondFloor = [];
List<RoutePositionModel> routePositionsSecondFloor = [];
LinesModel? lineSecondFloor;
List<LinesModel> linesSecondFloor = [];
pointersOnImage(Size? size) {
  List<Widget> returnList = [
    Image.asset(
      'assets/images/second_floor-copy.jpg',
  ...
  for (var element in shopPointersLocationsSecondFloor) {
    ...
    print('shop POINTERS LOCATIONS :: ==>> :$shopPointersLocationsSecondFloor');
  }

  return returnList;
}

class _SecondFloorUIState extends State<SecondFloorUI>
  ...
  void initState() {
    ...
    controller.forward();
    setState(() {
      routePositionsSecondFloor = PositionsController.routePositionModel;
      shopPointersLocationsSecondFloor = PositionsController.shopPostionModel;
      linesSecondFloor = PositionsController.lines;
      lineSecondFloor = PositionsController.line;
      print("shop pointers locs");
      print(shopPointersLocationsSecondFloor);
      print("lines");
      print(lineSecondFloor);
    });
  }
  ...
  Widget build(BuildContext context) {
    MyPainter painter = MyPainter(routePositionsSecondFloor);
    _onRouteSelect(TapUpDetails details) {
      var x = details.localPosition.dx.toInt();
      var y = details.localPosition.dy.toInt();
      Size size = currentSize;
      dynamic lineId = lineSecondFloor?.id;

      setState(() {
        routePositionsSecondFloor.add(RoutePositionModel(
            x: x,
            y: y,
            screenWidth: size.width.toInt(),
            screenHeight: size.height.toInt(),
            lineId: lineId));
        print('lines modell iddd : ${lineSecondFloor?.id ?? 0}');
      });
    ...
    onTapDown(TapUpDetails details) {
      ...
      setState(() {
        shopPointersLocationsSecondFloor.add(ShopPositionModel(
                            ...
                            children: [
                              Expanded(
                                child: GestureDetector(
                                  onTap: () {
                                    setState(() async {
                                      shopPointersLocationsSecondFloor
